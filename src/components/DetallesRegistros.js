...
const DetallesRegistros = ({registro}) =>{
    ...
    return (
        <div className=" rounded m-5 py-5 relative shadow-xl">
            ...
            <p className="m-0 px-2 text-base text-[#555]"><strong>Estado </strong>{registro.estado_ingreso}</p>
            <p className="m-0 px-2 text-base text-[#555]"> Hace {formatDistanceToNow(new Date(registro.createdAt), { locale: es }, { addSuffix: true })}</p>            
            <p className="m-0 px-2 text-base text-[#555]">{format(new Date(registro.createdAt), "dd MMMM  yyyy", { locale: es })}</p>
            <span className=" absolute top-5 right-5 cursor-pointer bg-white py-1.5 rounded-[50%] text-[#333] material-symbols-outlined" onClick={handleClick}>delete</span>
        </div>
    )
}
